...
enum class Side : bool { Buy = false, Sell = true };
...
struct tradeRecord {
  // tradeRecord has two orders (buy and sell, and the actual transactions
  // (trade struct))
  order order1;
  order order2;
  trade tradeDone;
...
enum class action : bool { modify = true, cancel = false };
