...
protected:
  ...
  {
    std::int32_t& bestPxIdx = (side == Side::Sell)
                                ? static_cast<Derived*>(this)->bestBidIdx
                                : static_cast<Derived*>(this)->bestAskIdx;
    auto& book = (side == Side::Sell) ? static_cast<Derived*>(this)->bidBook
                                      : static_cast<Derived*>(this)->askBook;
  ...
  std::vector<int32_t> def_addLimitOrder(
    orderReceived received,
    OrderPtr      cleanRecPt)
  {
    std::int32_t  priceIdx  = priceToIdx(received.price);
    std::int32_t& bestPxIdx = (received.side == Side::Sell)
                                ? static_cast<Derived*>(this)->bestBidIdx
                                : static_cast<Derived*>(this)->bestAskIdx;
    // If sell then make it priceIdx <= bestPxIdex
    bool condition = (received.side == Side::Sell) ? (priceIdx <= bestPxIdx)
                                                   : (priceIdx >= bestPxIdx);
    if (DEBUGMODE) {
    ...
    return (condition) ? static_cast<Derived*>(this)->matchOrder(
                           cleanRecPt, priceIdx, received.side, bestPxIdx)
                       : static_cast<Derived*>(this)->pushOrder(
                           cleanRecPt, priceIdx, received.side);
  }
  template<typename OrderLocation>
  void def_CheckLookUpMap(
    std::unordered_map<std::int32_t, OrderLocation>& lookUpMap)
  {
    for (auto& pair : lookUpMap) {
      ...
      if (key != loc.order_pt->order_id) {
        printf(
          "Error during checking the lookUpMap is legit for order %d \n", key);
        fflush(stdout);
        printf("Stop");
      }
    }
  ...
  std::vector<int32_t> def_pushOrder(
    book&        desiredBook,
    cleanRecType cleanRec,
    std::int32_t priceIdx,
    Side         side)
  {
    std::int32_t& bestPxIdx = (side == Side::Sell)
                                ? static_cast<Derived*>(this)->bestAskIdx
  ...
  void def_showBookImpl(BookList orderBook)
  {
    ...
    std::cout << "Best Ask: "
              << (static_cast<Derived*>(this)->bestAskIdx * TICKSIZE) + MINPRICE
              << std::endl;
    std::cout << "Best Bid: "
              << (static_cast<Derived*>(this)->bestBidIdx * TICKSIZE) + MINPRICE
              << std::endl;
  }
  void def_showLookUpMapImpl()
  {
    ...
    for (const auto& entry : static_cast<Derived*>(this)->lookUpMap) {
      ...
      std::cout << "Side: (Is Sell?) " << static_cast<bool>(entry.second.side)
                << std::endl;
      std::cout << "Price Index: "
                << (entry.second.price_index * TICKSIZE) + MINPRICE
                << std::endl;
      std::cout << "--------------------------" << std::endl;
